...
功能：
1. 读取脚本所在目录下的所有 .txt 文件，提取刀具名称和对应的 L 值
2. 遍历同目录下的 .xlsx 和 .xls 文件，在表格中查找对应的刀具名称
3. 将 L 值写入到对应的"装刀长"列中
...
def extract_tool_data_from_txt(txt_path):
    ...
    for line in lines:
        ...
        if tool_match:
            ...
            # 精确匹配 L= 后面的数值，注意区分 FL= 和 L=
            # 使用负向前瞻确保前面不是 F
            l_match = re.search(r'(?<!F)L=([0-9]+\.?[0-9]*)', line)
            if l_match:
                l_value_raw = l_match.group(1)

                # 去掉末尾的小数点（如 40. -> 40）
                l_value = l_value_raw.rstrip('.')

                # 存入字典
                if tool_name and l_value:
                    tool_data[tool_name] = l_value
...
def find_column_headers(ws, max_rows=30):
    """
    在表格前 max_rows 行中查找"刀具名称"和"装刀长"的列号
    返回: (刀具名称列号, 装刀长列号, 表头行号)
    如果找不到返回 None
    """
    tool_name_col = None
    ...
    for row in range(1, max_rows + 1):
        for col in range(1, 21):
            try:
                ...
                # 如果两个都找到了，提前返回
                if tool_name_col is not None and tool_length_col is not None:
                    return tool_name_col, tool_length_col, header_row
...
def process_excel_file(excel_path, tool_data):
    ...
    try:
        ...
        tool_name_col, tool_length_col, header_row = header_result
        ...
        for row in range(data_start_row, max_search_row + 1):
            ...
            if tool_name in tool_data:
                l_value_str = tool_data[tool_name]
                try:
                    # 转换为 float
                    l_value = float(l_value_str)

                    # 写入到装刀长列
                    ws.Cells(row, tool_length_col).Value = l_value
                    updated_count += 1
                except ValueError:
                    pass
